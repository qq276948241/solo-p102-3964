...
export const TEMPERATURE_OPTIONS: { value: Temperature; label: string; icon: string }[] = [
  { value: 'iced', label: '冰', icon: '🧊' },
  { value: 'hot', label: '热', icon: '🔥' }
];

export const CUPSIZE_OPTIONS: { value: CupSize; label: string; suffix: string; priceDelta: number }[] = [
...
export const LARGE_CUP_EXTRA = 3;

export const hasSpecOptions = (category: CategoryType): boolean => {
  return category === 'latte' || category === 'americano' || category === 'pour-over';
};
...
export const calculatePrice = (basePrice: number, spec?: ProductSpec): number => {
  if (!spec) return basePrice;
  const sizeOption = CUPSIZE_OPTIONS.find(o => o.value === spec.cupSize);
  return basePrice + (sizeOption?.priceDelta ?? 0);
};

export const formatSpecLabel = (spec?: ProductSpec): string => {
  if (!spec) return '';
  const tempLabel = TEMPERATURE_OPTIONS.find(o => o.value === spec.temperature)?.label ?? '';
  const sizeLabel = CUPSIZE_OPTIONS.find(o => o.value === spec.cupSize)?.label ?? '';
